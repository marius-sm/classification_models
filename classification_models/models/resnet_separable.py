...
def residual_conv_block(filters, stage, block, strides=(1, 1), attention=None, cut='pre', group_norm=True, batch_norm=False):
    ...
    def layer(input_tensor):
        ...
        if not group_norm and not batch_norm:
            x = input_tensor
        ...
        x = layers.SeparableConv2D(filters, (3, 3), strides=strides, name=conv_name + '1', **conv_params)(x)
        ...
        x = layers.ZeroPadding2D(padding=(1, 1))(x)
        x = layers.SeparableConv2D(filters, (3, 3), name=conv_name + '2', **conv_params)(x)
